...
export const quizAPI = {
  ...
  async getRooms(params = {}) {
    const response = await apiV1Client.get('/quiz/rooms', { params });
    return response.data;
  },

  // Get quiz room details with questions
  async getRoom(roomId) {
    const response = await apiV1Client.get(`/quiz/rooms/${roomId}`);
    return response.data;
  },

  // Create quiz room (teachers/admins)
  async createRoom(roomData) {
    const response = await apiV1Client.post('/quiz/rooms', roomData);
    return response.data;
  },

  // Add question to room (teachers/admins)
  async addQuestion(roomId, questionData) {
    const response = await apiV1Client.post(`/quiz/rooms/${roomId}/questions`, questionData);
    return response.data;
  },

  // Start quiz session
  async startSession(roomId) {
    const response = await apiV1Client.post('/quiz/sessions/start', { room_id: roomId });
    return response.data;
  },

  // Submit answer
  async submitAnswer(sessionId, answerData) {
    const response = await apiV1Client.post(`/quiz/sessions/${sessionId}/answer`, answerData);
    return response.data;
  },

  // Complete quiz session
  async completeSession(sessionId) {
    const response = await apiV1Client.post(`/quiz/sessions/${sessionId}/complete`);
    return response.data;
  },

  // Get leaderboard
  async getLeaderboard(roomId, limit = 10) {
    const response = await apiV1Client.get(`/quiz/rooms/${roomId}/leaderboard`, { params: { limit } });
    return response.data;
  },

  // Get session answers
  async getSessionAnswers(sessionId) {
    const response = await apiV1Client.get(`/quiz/sessions/${sessionId}/answers`);
    return response.data;
  },

  // Delete quiz room (teachers/admins)
  async deleteRoom(roomId) {
    const response = await apiV1Client.delete(`/quiz/rooms/${roomId}`);
    return response.data;
  }
};
...
export const chatRoomAPI = {
  // Get all chat rooms
  async getRooms() {
    const response = await apiV1Client.get('/chat/rooms');
    return response.data;
  },

  // Get chat room details
  async getRoom(roomId) {
    const response = await apiV1Client.get(`/chat/rooms/${roomId}`);
    return response.data;
  },

  // Create chat room
  async createRoom(roomData) {
    const response = await apiV1Client.post('/chat/rooms', roomData);
    return response.data;
  },

  // Join chat room by code
  async joinRoom(roomCode, nickname = null) {
    const response = await apiV1Client.post('/chat/rooms/join', { room_code: roomCode, nickname });
    return response.data;
  },

  // Update participant settings
  async updateParticipant(participantId, updateData) {
    const response = await apiV1Client.patch(`/chat/participants/${participantId}`, updateData);
    return response.data;
  },

  // Send message
  async sendMessage(roomId, messageData) {
    const response = await apiV1Client.post(`/chat/rooms/${roomId}/messages`, messageData);
    return response.data;
  },

  // Get messages
  async getMessages(roomId, limit = 50, offset = 0) {
    const response = await apiV1Client.get(`/chat/rooms/${roomId}/messages`, { 
      params: { limit, offset } 
    });
    return response.data;
  },
  ...
  async getParticipants(roomId) {
    const response = await apiV1Client.get(`/chat/rooms/${roomId}/participants`);
    return response.data;
  },

  // Update room theme
  async updateTheme(roomId, theme) {
    const response = await apiV1Client.patch(`/chat/rooms/${roomId}/theme`, { theme });
    return response.data;
  },

  // Delete chat room
  async deleteRoom(roomId) {
    const response = await apiV1Client.delete(`/chat/rooms/${roomId}`);
    return response.data;
  }
};
