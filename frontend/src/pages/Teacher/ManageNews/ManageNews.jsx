...
const ManageNews = () => {
  ...
  const loadNews = async () => {
    try {
      setLoading(true);
      const res = await apiClient.get('/api/v1/news/manage/all');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingNews) {
        await apiClient.put(`/api/v1/news/${editingNews.id}`, formData);
        alert('Cập nhật tin tức thành công!');
      } else {
        await apiClient.post('/api/v1/news/', formData);
      ...
      alert('Lỗi: ' + (error.response?.data?.detail || 'Không thể lưu tin tức'));
    }
  };
  ...
  return (
    <div className="manage-news-page">
      <div className="page-header">
        <div>
          <h1 className="page-title">📰 Quản lý Tin tức của tôi</h1>
          <p className="page-subtitle">Tạo và quản lý tin tức, bài viết chia sẻ kinh nghiệm giảng dạy</p>
        </div>
        ...
        <div className="news-table-container">
          ...
          {newsList.length === 0 && (
            <div className="empty-state">
              ...
              <p>Tạo tin tức đầu tiên để chia sẻ kinh nghiệm của bạn</p>
            </div>
          )}
        </div>
      )}
