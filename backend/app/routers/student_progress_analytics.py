...
def extract_error_patterns(submissions: list[Submission]) -> list[ErrorPattern]:
    ...
    for sub in submissions:
        ...
        try:
            ...
            errors = json.loads(sub.error_analysis) if isinstance(sub.error_analysis, str) else sub.error_analysis
...
def classify_exercise_type(exercise: Exercise, exam: ExamAssessment | None = None) -> ExerciseType:
...
async def export_weekly_report(
    class_id: int,
    week_start: datetime | None = Query(None, description="Week start date (defaults to last Monday)"),
