...
logger = logging.getLogger(__name__)
    ...
    def calculate_speaking_score(self, assessment_result: dict, max_score: float = 10.0) -> dict:
        ...
        if pronunciation >= 80:
            feedback_parts.append("? Phát âm r?t t?t")
        elif pronunciation >= 60:
            feedback_parts.append("?? Phát âm c?n c?i thi?n")
        else:
            feedback_parts.append("? Phát âm c?n luy?n t?p nhi?u hon")
        
        if fluency >= 80:
            feedback_parts.append("? Nói trôi ch?y t? nhiên")
        elif fluency >= 60:
            feedback_parts.append("?? C?n nói t? nhiên hon")
        else:
            feedback_parts.append("? C?n luy?n t?p d? nói trôi ch?y hon")
        
        if completeness >= 80:
            feedback_parts.append("? Hoàn thành d?y d? n?i dung")
    ...
    def _generate_detailed_feedback(self, pronunciation: float, fluency: float, completeness: float, accuracy: float, reference_text: str = "", recognized_text: str = "", words_detail: list = None) -> str:
        ...
        try:
            ...
            prompt = f"""B?n là giáo viên ti?ng Anh dang ch?m bài nói c?a h?c sinh. Hãy dua ra nh?n xét chi ti?t b?ng ti?ng Vi?t d?a trên k?t qu? dánh giá phát âm t? Azure Speech API.

**Thông tin dánh giá:**
- Ði?m phát âm (Pronunciation): {pronunciation:.1f}/100
- Ði?m d? trôi ch?y (Fluency): {fluency:.1f}/100
- Ði?m hoàn thi?n (Completeness): {completeness:.1f}/100
- Ði?m chính xác (Accuracy): {accuracy:.1f}/100

**N?i dung yêu c?u:** {reference_text if reference_text else "Không có"}
**N?i dung h?c sinh nói:** {recognized_text if recognized_text else "Không nh?n di?n du?c"}

Hãy dua ra nh?n xét chi ti?t v?i c?u trúc sau:

1. **T?ng quan**: Ðánh giá chung v? bài nói (2-3 câu)

2. **Phát âm (Pronunciation {pronunciation:.1f}/100)**:
   - Ði?m m?nh
   - Ði?m c?n c?i thi?n (n?u có)
   - L?i khuyên c? th?

3. **Ð? trôi ch?y (Fluency {fluency:.1f}/100)**:
   - Nh?n xét v? nh?p di?u, t?c d? nói
   - G?i ý c?i thi?n

4. **Tính hoàn ch?nh (Completeness {completeness:.1f}/100)**:
   - Ðánh giá m?c d? hoàn thành n?i dung
   - Nh?ng ph?n còn thi?u (n?u có)

5. **L?i khuyên**: 2-3 l?i khuyên thi?t th?c d? c?i thi?n k? nang nói

Vi?t theo phong cách d?ng viên, khích l? h?c sinh. Dùng emoji phù h?p. Gi?i h?n kho?ng 200-300 t?."""
    ...
    def _generate_template_feedback(self, pronunciation: float, fluency: float, completeness: float, accuracy: float) -> str:
        ...
        if pronunciation >= 80:
            feedback.append("- Phát âm chu?n xác, rõ ràng")
        elif pronunciation >= 60:
            feedback.append("- C?n chú ý phát âm m?t s? t? cho chu?n hon")
        else:
            feedback.append("- Nên luy?n t?p phát âm các t? khó, nghe và l?p l?i nhi?u l?n")
        
        feedback.append(f"\n**Ð? trôi ch?y (Fluency):** {fluency:.1f}/100")
        if fluency >= 80:
            feedback.append("- Nói trôi ch?y, t? nhiên")
        elif fluency >= 60:
            feedback.append("- Có th? ng?t quãng ? m?t s? ch?, c?n luy?n t?p d? t? nhiên hon")
        else:
            feedback.append("- Nên d?c to nhi?u l?n d? quen v?i nh?p di?u và t?c d? nói")
        
        feedback.append(f"\n**Tính hoàn ch?nh (Completeness):** {completeness:.1f}/100")
        if completeness >= 80:
            feedback.append("- Hoàn thành d?y d? n?i dung yêu c?u")
        elif completeness >= 60:
            feedback.append("- Thi?u m?t s? ph?n, hãy d?m b?o d?c/nói h?t n?i dung")
        else:
            feedback.append("- Chua hoàn thành d? n?i dung, c?n d?c/nói d?y d? hon")
        
        feedback.append(f"\n**Ð? chính xác (Accuracy):** {accuracy:.1f}/100")
