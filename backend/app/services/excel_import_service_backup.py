...
CHỈ trả về JSON, không giải thích."""
    ...
    def parse_excel_file(file: UploadFile) -> list[StudentExcelRow]:
        ...
        try:
            ...
            if df is None:
                print("⚠️ All pandas engines failed, trying AI parsing...")
            ...
            if header_row_index is None and len(df.columns) >= 3:
                ...
                if has_ma_hs or has_ho_ten:
                    print("ℹ️  File đã có header, không cần tìm header row")
                    header_row_index = -1  # Đánh dấu là đã có header
                else:
                    print("⚠️  Không tìm thấy header, giả định dòng 0 là header")
                    header_row_index = 0
            ...
            if ma_hs_col == ten_hs_col:
                ...
                print("   This is wrong! Trying to fix...")
            ...
            if len(df) > 0:
                ...
                if 'mã' in sample_ma.lower() or 'ma' in sample_ma.lower() or sample_ma.lower() == 'nan':
                    print("⚠️  First row still looks like header, skipping it")
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    if ma_hs == ten_hs:
                        ...
                        print("   This means columns are detected INCORRECTLY!")
                        print("   Skipping this row or using AI parsing...")
                        continue
                    ...
                    if ten_hs.isdigit() and len(ten_hs) == 10:
                        ...
                        print("   This suggests wrong column detection. Skipping...")
                        continue
            ...
            try:
                ...
                error_msg = "Không thể đọc file Excel.\n\n"
                error_msg += f"📋 Lỗi pandas: {type(e).__name__}: {str(e)}\n"
                error_msg += f"🤖 AI parsing cũng thất bại: {type(ai_error).__name__}: {str(ai_error)}\n\n"
                error_msg += "💡 Giải pháp:\n"
                error_msg += "1. Kiểm tra file có đúng format: STT, Mã học sinh, Họ và tên\n"
                error_msg += "2. Thử convert file sang .xlsx (Excel mới) bằng Microsoft Excel/LibreOffice\n"
                error_msg += "3. Tải file mẫu và copy dữ liệu vào\n"
                error_msg += "4. Đảm bảo file không bị password protection"
