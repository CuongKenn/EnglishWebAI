...
def transObjs(matrix, objs, scale):
    ...
    points:
        row[0]: x
        row[1]: y
        row[2]: match cv2.getRotationMatrix2D output dimension
        colume[0]: center point
        colume[1~5]: landmarks
    """
    ...
    points = np.matmul(matrix, points)      # change center point & landmarks for each obj
    newobjs = []
    for index, obj in enumerate(objs):
        ...
        cls = determine_cls(x, y, r, b, width, height, obj.classification)  # process exceed image size
        ...
        newobjs.append(BBox(cls=cls, xyrb=[x, y, r, b], landmark=landmark, rotate=obj.rotate))
    return newobjs
...
def computeIoUMin(rec1, rec2):
    ...
    S_rec2 = (gx2 - gx1) * (gy2 - gy1)  # area of G
    ...
    w = max(0, x2 - x1)
    h = max(0, y2 - y1)
    area = w * h  # area of C & G
    miniou = min(S_rec1, S_rec2)
    if miniou == 0:
        return 0
    else:
        iou = area / miniou
...
def cubeTransform(image, objs, outw, outh, keepsize=8):
    """
    Create a square image to input the models.
    If the objs area is smaller than 8*8 pixels, delete it.
    """
...
def augmentWithCropScaleWebface(image, objs, outw=800, outh=800, mode='normal', keepsize=8):
    ...
    mode: normal
        (80%) scale = 0.5 ~ 2.0
        (20%) scale = 1
        change image center point according to width or height = 0.1 ~ 0.9
        (50%) rotate angle -45 ~ 45

    mode: cube
        (50%) rotate angle -45 ~ 45
              scale = 0.9 ~ 1.2
              change image center point according to width or height = 0.4 ~ 0.6

    Use cv2.getRotationMatrix2D get rotation matrix.    cv2.getRotationMatrix2D((cx, cy), angle, scale)
    ...
    for i in range(len(objs) - 1, -1, -1):
        ...
        if ioumin < 0.15 or objs[i].area < keepsize * keepsize:
            # if area of obj is smaller than 8*8 pixels, detect the obj.
            del objs[i]

    if len(objs) == 0 and randrf(0, 1) > 0.5:
        # (50%) Run again if number of objs is 0
